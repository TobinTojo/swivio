-- Swivio: run this once in Supabase SQL Editor if you see 404/400 errors
-- (user_profiles missing, room_users is_ready/avatar_url missing, chat/daily tables missing)
-- Safe to re-run — uses IF NOT EXISTS / IF EXISTS where possible.
-- Ignore errors like "already member of publication" on re-run.

-- ── Lobby columns (ready-up + avatars) ─────────────────────────────────────
...
-- ── Daily swipes, profiles, chat, daily picks ───────────────────────────────
...
drop policy if exists "profiles_select_auth" on user_profiles;
drop policy if exists "profiles_upsert_self" on user_profiles;
...
drop policy if exists "user_swipes_select_auth" on user_swipes;
drop policy if exists "user_swipes_insert_self" on user_swipes;
...
drop policy if exists "messages_select_auth" on room_messages;
drop policy if exists "messages_insert_auth" on room_messages;
...
drop policy if exists "daily_picks_select_auth" on room_daily_picks;
drop policy if exists "daily_picks_insert_auth" on room_daily_picks;
...
drop policy if exists "daily_votes_select_auth" on room_daily_votes;
drop policy if exists "daily_votes_insert_self" on room_daily_votes;
drop policy if exists "daily_votes_update_self" on room_daily_votes;
